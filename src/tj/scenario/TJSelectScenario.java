...
public class TJSelectScenario extends XScenario {
    ...
    public void updateSelectedPtCurves() {
        ...
        TJPage[] pagesToCheck = tj.getJournalBookMgr().getCurPage();
        if (pagesToCheck != null) {
            System.out.println("\n--- Checking curves from pages ---");
            for (TJPtCurve ptCurve : pagesToCheck[0].getPtCurves()) {
                Rectangle2D curveBounds = ptCurve.getBoundingBox();
            ...
            for (TJPtCurve ptCurve : pagesToCheck[1].getPtCurves()) {
                Rectangle2D curveBounds = ptCurve.getBoundingBox();
        ...
        TJPage[] currentPages = tj.getJournalBookMgr().getCurPage();
        if (currentPages != null) {
            // Clear page selections
            currentPages[0].getSelectedPtCurves().clear();
            currentPages[1].getSelectedPtCurves().clear();
            // Add selected curves that belong to each page
            for (TJPtCurve selectedCurve : tj.getPtCurveMgr().getSelectedPtCurves()) {
                if (currentPages[0].getPtCurves().contains(selectedCurve)) {
                    currentPages[0].getSelectedPtCurves().add(selectedCurve);
                    System.out.println("  Added to page[0]: " + selectedCurve);
                } else if (currentPages[1].getPtCurves().contains(selectedCurve)) {
                    currentPages[1].getSelectedPtCurves().add(selectedCurve);
                    System.out.println("  Added to page[1]: " + selectedCurve);
                }
            }
            System.out.println("Page[0] selected: " + currentPages[0].getSelectedPtCurves().size());
            System.out.println("Page[1] selected: " + currentPages[1].getSelectedPtCurves().size());
        }
    }
